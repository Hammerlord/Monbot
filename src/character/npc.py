...
class NPC(Character):
    ...
    def __init__(self):
        super().__init__()
        self.is_npc = True
        self.profession = 0  # An enumerator
        self._potential_species = []  # List[Species] -- TBD by descendants

    def _generate_team(self, opponent: Character) -> None:
        """
    ...
    def _generate_level(self, opponent: Character) -> None:
        ...
        """
        min_level = max(1, opponent.get_level() - 2)  # Cannot be below 1
        max_level = opponent.get_level() + 1
    ...
    def _roll_team_size(opponent: Character) -> int:
        ...
        """
        min_team_size = 1
        max_team_size = opponent.get_team().get_size()
